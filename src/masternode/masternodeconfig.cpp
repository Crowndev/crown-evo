...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        FILE* configFile = fsbridge::fopen(pathMasternodeConfigFile.string().c_str(), "a");
        if (configFile) {
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
            if (port != nDefaultPort) {
                strErr = "Invalid port detected in masternode.conf\n";
                streamConfig.close();
                return false;
            }
...
bool CMasternodeConfig::CMasternodeEntry::castOutputIndex(int& n) const
{
    try {
        ...
        LogPrint(BCLog::MASTERNODE, "%s: %s on getOutputIndex\n", __func__, e.what());
        return false;
    }
